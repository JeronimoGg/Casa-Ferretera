...
import FormularioEditarSupervisor from "@/components/formulario/editarSupervisor";

const editarSupervisor = ( { params }: { params: { id: number } }) => {
    const [data, setData] = useState<any>({})
    const [error, setError] = useState<string | undefined>(undefined);
    const [rol, setRol] = useState<string | undefined>(undefined);
    const titulo = "Editar supervisor";

    const getInfoSupervisor = async (id: number) => {
        try {
    ...
    return(
        <LayoutProfile titulo={titulo}> 
            ...
            {Object.keys(data).length > 0 && <FormularioEditarSupervisor initialValues={initialValues} id={data.id_supervisor} />}
        </LayoutProfile>
    )
}
